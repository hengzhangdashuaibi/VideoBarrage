...
export function getToken() {
  return localStorage.getItem('REMEMBER') ? localStorage.getItem('token') : sessionStorage.getItem('token');
}
export function getExpires() {
  const ex = localStorage.getItem('REMEMBER') ? localStorage.getItem('expires') : sessionStorage.getItem('expires');
  const date = Date.parse(new Date()) / 1000;
  return ex - date;
}
export function getRefreshToken() {
  return localStorage.getItem('REMEMBER') ? localStorage.getItem('refresh_token') : sessionStorage.getItem('refresh_token');
}
export function getDicByCode(data, code) {
  return data[code];
}
export function getDicByKey(data, code, key) {
  return data[code] ? (data[code].find(d => d.key === key) ? data[code].find(d => d.key === key).value : key) : key;
}
